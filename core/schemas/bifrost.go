...
const (
	...
	BifrostContextKeyPreRequestShortCircuitError         BifrostContextKey = "bifrost-prerequest-short-circuit-error"           // *BifrostError set by PreRequestHook plugins that must fail before provider validation.
	...
	BifrostContextKeyParentRequestID                     BifrostContextKey = "bifrost-parent-request-id"                        // string (parent linkage for grouped request logs)
	...
	BifrostContextKeyStreamFirstByteTime                 BifrostContextKey = "bifrost-stream-first-byte-time"             // time.Time of the first raw streaming data payload from provider
